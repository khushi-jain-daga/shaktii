...
export default function FilesPage() {
  ...
  async function run(id: string, action: 'encrypt' | 'verify') {
    setBusyId(id);
    setError('');
    try {
      if (action === 'encrypt') await nodeApi.encrypt(id);
      else await nodeApi.verify(id);
      await load();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-white/50">Upload, encrypt and verify file integrity from the Node.js security service.</p>
        </div>
