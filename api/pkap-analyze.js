...
async function withTimeout(promise, ms) {
  let timer;
  try {
    return await Promise.race([promise, new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('Provider timeout')), ms); })]);
  } finally {
    clearTimeout(timer);
  }
}
...
async function grok(prompt) {
  ...
  const groq = process.env.GROK_API_KEY.startsWith('gsk_');
  const endpoint = groq ? 'https://api.groq.com/openai/v1/chat/completions' : 'https://api.x.ai/v1/chat/completions';
  const model = groq ? 'llama-3.3-70b-versatile' : 'grok-beta';
  const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.GROK_API_KEY}` }, body: JSON.stringify({ model, messages: [{ role: 'user', content: prompt }], temperature: 0, response_format: { type: 'json_object' } }) });
  if (!response.ok) throw new Error(`Grok/Groq ${response.status}`);
  const payload = await response.json();
...
async function claude(prompt) {
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': process.env.ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01' }, body: JSON.stringify({ model: 'claude-3-haiku-20240307', max_tokens: 4000, messages: [{ role: 'user', content: prompt }] }) });
  if (!response.ok) throw new Error(`Claude ${response.status}`);
  const payload = await response.json();
  return parseJson(payload?.content?.[0]?.text || '{}');
}
...
export default async function handler(req, res) {
  ...
  if (!redactedData) return res.status(400).json({ success: false, error: 'redactedData is required' });
  const sample = redactedData.length > 30000 ? `${redactedData.slice(0, 15000)}\n...[TRUNCATED]...\n${redactedData.slice(-15000)}` : redactedData;
  const prompt = `You are a Tier 3 Security Operations Center analyst working inside SHAKTII Pkap Analyzer. Analyze the privacy-redacted log and be precise. Do not invent compromise, attribution, malware names or assets not supported by evidence. ${schemaPrompt}\n\nFile: ${fileName}\n\nRedacted log:\n${sample}`;
  const providers = [['Gemini', gemini, 35000], ['Grok/Groq', grok, 25000], ['Claude', claude, 25000], ['OpenAI', openai, 25000]];
  const errors = [];
  ...
  return res.status(503).json({ success: false, error: 'All configured AI providers are unavailable. Client-side deterministic analysis will be used.', providerErrors: errors, aiFallback: true });
}
